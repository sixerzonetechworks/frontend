 ...
 * @param {string} date - Date in YYYY-MM-DD format
 * @param {number} startHour - Hour (0-23)
 * @returns {Promise<Array>} Array of ground objects
 * @example
 * [
 *   {"id": 1, "name": "G1", "location": "Downtown", "available": true},
 *   {"id": 2, "name": "G2", "location": "North Zone", "available": false}
 * ]
 */
export const getAvailableGrounds = async (date, startHour) => {
  try {
    const response = await fetch(`${API_BASE_URL}/grounds/get-available-grounds?date=${date}&startHour=${startHour}`);
